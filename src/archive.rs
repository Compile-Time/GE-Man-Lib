...
//!
//! This module defines operations that work with compressed archives from GloriousEgroll's Proton and Wine
//! releases.
use std::io;
use std::io::Read;
...
///
/// Generates a checksum from the provided `compressed_archive` file and compares it to the `expected_sum`. This method
/// attempts to split the expected sum by whitespaces before comparing it with the generated checksum from
/// `compressed_archive`. This is done because GE releases provide checksums with the sha512sum tool which also outputs
...
mod checksum_tests {
    ...
    fn check_if_equal_checksums_where_expected_sum_contains_file_name_match() {
        let tar = fs::read("test_resources/assets/test.tar.gz").unwrap();
        let checksum =
...
mod extraction_tests {
    ...
    fn extract_proton_ge_release_with_correct_tag_kind() {
        ...
        let dst = super::extract_compressed(&kind, archive, tmp_dir.path()).unwrap();
    ...
    fn extract_wine_ge_release_with_correct_tag_kind(kind: TagKind) {
        ...
        let archive = File::open("test_resources/assets/test.tar.xz").unwrap();
